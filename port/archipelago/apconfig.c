...
#define AP_CONFIG_MAX_SETTINGS 450
...
} ap_settings[AP_CONFIG_MAX_SETTINGS];

static s32 numSettings = 0;
static u8 configMaxWarningLogged = 0;

static inline s32 APConfigClampInt(s32 val, s32 min, s32 max)
{
	return (val < min) ? min : ((val > max) ? max : val);
}

static inline u32 APConfigClampUInt(u32 val, u32 min, u32 max)
{
	return (val < min) ? min : ((val > max) ? max : val);
}

static inline f32 APConfigClampFloat(f32 val, f32 min, f32 max)
{
	return (val < min) ? min : ((val > max) ? max : val);
}

static inline struct configentry *APConfigFindEntry(const char *key)
{
	for (s32 i = 0; i < numSettings; ++i) {
		if (!strncasecmp(ap_settings[i].key, key, CONFIG_MAX_KEYNAME)) {
			return &ap_settings[i];
		}
	}
	return NULL;
}

static inline struct configentry *APConfigAddEntry(const char *key)
{
	if (numSettings < AP_CONFIG_MAX_SETTINGS) {
		struct configentry *cfg = &ap_settings[numSettings++];
	...
	if (!configMaxWarningLogged) {
		sysLogPrintf(LOG_WARNING, "Maximum number of configuration entries exceeded: %d", AP_CONFIG_MAX_SETTINGS);
		configMaxWarningLogged = 1;
	}
	return NULL;
}

static inline struct configentry *APConfigFindOrAddEntry(const char *key)
{
	for (s32 i = 0; i < numSettings; ++i) {
		if (!strncasecmp(ap_settings[i].key, key, CONFIG_MAX_KEYNAME)) {
			return &ap_settings[i];
	...
	return APConfigAddEntry(key);
}

static inline const char *APConfigGetSection(char *sec, const struct configentry *cfg)
{
...
static void APConfigSaveEntry(struct configentry *cfg, FILE *f)
{
	switch (cfg->type) {
		case CFG_S32:
			if (cfg->min_s32 < cfg->max_s32) {
				*(s32 *)cfg->ptr = APConfigClampInt(*(s32 *)cfg->ptr, cfg->min_s32, cfg->max_s32);
			}
			fprintf(f, "%s=%d\n", cfg->key + cfg->seclen + 1, *(s32 *)cfg->ptr);
			break;
		case CFG_F32:
			if (cfg->min_f32 < cfg->max_f32) {
				*(f32 *)cfg->ptr = APConfigClampFloat(*(f32 *)cfg->ptr, cfg->min_f32, cfg->max_f32);
			}
			fprintf(f, "%s=%f\n", cfg->key + cfg->seclen + 1, *(f32 *)cfg->ptr);
			break;
		case CFG_U32:
			if (cfg->min_u32 < cfg->max_u32) {
				*(u32*)cfg->ptr = APConfigClampUInt(*(u32*)cfg->ptr, cfg->min_u32, cfg->max_u32);
			}
...
s32 APConfigSave(const char *fname)
{
	...
	APConfigGetSection(curSec, &ap_settings[0]);
	fprintf(f, "[%s]\n", curSec);

	for (s32 i = 0; i < numSettings; ++i) {
		struct configentry *cfg = &ap_settings[i];
		APConfigGetSection(tmpSec, cfg);
		if (strncmp(curSec, tmpSec, CONFIG_MAX_SECNAME) != 0) {
			fprintf(f, "\n[%s]\n", tmpSec);
			strncpy(curSec, tmpSec, CONFIG_MAX_SECNAME);
		}
		APConfigSaveEntry(cfg, f);
	}
...
s32 APConfigLoad(const char *fname)
{
	...
	while (fgets(lineBuf, sizeof(lineBuf), f)) {
		...
		if (token[0] == '[' && token[1] == '\0') {
			...
			APConfigSetFromString(keyBuf, line);
		}
	}
...
{
	if (fsFileSize("$S/ap.ini") > 0) {
		APConfigLoad("$S/ap.ini");
	}
}
